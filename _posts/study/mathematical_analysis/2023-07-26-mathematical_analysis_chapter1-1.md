...
다음과 같은 방정식 
\\[p^2=2\\]를 
만족시키는 유리수 $p$는 존재하지 않는다. 그러한 $p$가 존재한다면, 우리는 둘 다 짝수가 아닌 어떤 정수들 $m$과 $n$을 이용하여  $p=m/n$ 와 같이 쓸 수 있을 것이다. 이러한 표현이 가능하다고 가정하자.
...
수학에 관심이 있는 사람이라면, 이 증명을 본적이 있을 것이다. 나도 고등학교 때 본 적이 있는 증명이다. (작성중)
